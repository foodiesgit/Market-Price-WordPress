...
{ 
	...
	public function is_localhost($domain=''){
		return in_array( (!empty($domain) ? $domain : $this->domainWithoutPortCurrent), ['localhost','127.0.0.1','::1']); 
	}
	...
	public function telegram_message($array, $botid, $repeated_call=false){
		...
		$res = $this->get_remote_data_array('https://api.telegram.org/bot'.$botid.'/sendMessage', $array);  //'sendMessage?'.http_build_query($array, '');
		if (!$res->error){
			$answer = $res->response;
	...
	public function get_remote_data($url, $post_paramtrs=false,             $request_options=[])	
	{
		...
		if (!$is_wp || $this->property('get_remote_data_ENABLE'))
		{
			if (is_callable('parent::get_remote_data')) { 
				return parent::get_remote_data($url, $post_paramtrs, $request_options);
			}
			else if (method_exists(get_parent_class(), 'get_remote_data')) { 
				$name= get_parent_class();
				$class = new $name();
				return $class->get_remote_data($url, $post_paramtrs, $request_options);
			}
	...
	public function get_remote_data_array($url, $post_paramtrs=false,  	$request_options=[], $should_be_json=false)
	{
		$data = $this->get_remote_data($url, $post_paramtrs, $request_options);
		if (empty($data))
		{
			return (object)['error'=>'empty data', 'response'=>''];
		}
		else{
			if ($should_be_json)
			{
				try { 
					return (object)['error'=>false, 'response'=>json_decode($data) ];
				}
				catch(\Exception $ex){
					return (object)['error'=>"not json", 'response'=>($data) ];
				}
			}
			else{
				return (object)['error'=>false, 'response'=>($data) ];
			}
		}
	}
	...
	public function asyncFunctions($callbacksArray, $which_method='spatie', $blocking=true){
		...
		$example = 'CALLBACK & ARGUMENT pair, like [$callback,$args] or [$callback], where $args behaves like as in call_user_func_array';
	...
	public function init_defaults()
	{
		...
		$this->is_https			= ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS']!=='off') || (!empty($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT']==443) );
		$this->https			= $this->is_https ? 'https://' : 'http://';
		$this->domainCurrent	= !empty($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '';
		$this->domain			= $this->https . $this->domainCurrent;
		$this->requestURI		= $this->array_value($_SERVER,'REQUEST_URI'); $this->requestURL=$this->requestURI;
		$this->currentURL		= $this->domain.$this->requestURI; 
		$this->domainWithoutPortCurrent= $this->array_value( parse_url($this->currentURL),'host');
		$this->is_localhost     = $this->is_localhost(); 
		// others 
...
class standard_wp_library extends standard_php_library  
{
	...
	*/
	...
	public function check_timer_remains($transientName, $seconds){
		$remains = 0;
		if ( ! empty($seconds) )
		{
			$transientName = "timerflag_".$transientName;
			$last_time = $this->get_transient($transientName,0);
			if( $last_time ) { 
				$remains = ($last_time + $seconds) - time();
			}
			else{
				set_transient($transientName, time(), $seconds);
				$remains = 0;
			}
		}
		return $remains;
	}
	public function check_timer_firstrun($uniqueActionId, $seconds){
		$uniqueActionId =  "px_firstrun_flag_".$uniqueActionId;
		if( $this->get_transient($uniqueActionId, null) === null ) { 
			$this->set_transient($uniqueActionId, time(), $seconds);
			return true;
		}
		return false;
	}
	public function check_firstrun($uniqueActionId){
		$uniqueActionId =  "px_firstrun_flag_".$uniqueActionId;
		if( !(get_option($uniqueActionId)) ) { 
			update_option($uniqueActionId, time());
			return true;
		}
		return false;
	}
...
class default_plugin 
{
	...
	public function nonce($action=null, $slug=null)
	{
		wp_nonce_field( $this->defNonceAction($action), $this->defNonceSlug($slug) );
	}
	public function nonceSubmit($text=null, $action=null, $slug=null, $centeredFloat=false)
	{
		$this->nonce( $this->defNonceAction($action), $this->defNonceSlug($slug) );
		if ($centeredFloat) echo '<div class="centered-float submitbutton">';
		submit_button($text, 'button-primary', '', true, $attrib= ['id'=>'mainsubmit-button'] );
		if ($centeredFloat) echo '</div>';
	}
	// old
