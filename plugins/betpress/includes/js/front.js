					...
					jQuery.post(ajaxurl,{
						...
						if(response['success'] == 1)
						{
							...
							if(response['results'].length > 0)
							{
								...
								betOdds[event_id] = response.results[0];
								makeTabs(event_id,home,away);
							}
						}
	...
	for (const item in oddData) {
		...
		str += `<div class="cat-containerR" style="background-color:#585858; color: #fff ; border-top: 2px solid #367a65;"> 
					...
					<div class="cat-wrapperR collapse" style="width:100%; margin-left: 0px; margin-bottom: 0px;" role="group" aria-label="Basic example" id=${event_id+oddData[item]['id']}>
						<div class="cat-container" style="width:100%">`;
		if((oddData[item]['name'] === "Full Time Result") || (oddData[item]['name'] === "To Qualify") || (oddData[item]['name'] === "Full Time Result – Enhanced Prices") || (oddData[item]['name'] === "10 Minute Result") || (oddData[item]['name'] === "Draw No Bet") || (oddData[item]['name'] === "Half Time Result") || (oddData[item]['name'] === "Race to 20 Points") || (oddData[item]['name'] === "1st Half Money Line 3-Way") || (oddData[item]['name'] === "2rd Half Money Line 3-Way"))
		{
			for(var i = 0; i< oddData[item]['odds'].length; i++){
				let count = 0;
				for(var j = 0; j < oddNames.length; j++){
					if(oddNames[j] == oddData[item]['odds'][i]['name']){
						count += 1;
						break;
					}
				}
				if(count == 0){
					oddNames.push(oddData[item]['odds'][i]['name']);
				}
				// str +=	`<div style="color:yellow"> ${oddData[item]['odds'][i]['odds']}</div>`;
			}
			str += `<div class="j-option-wrapper" style="width:100%;">
						<div class = "oddsByName" style = "display: flex">`;
			for(let i = 0; i < oddNames.length; i++){
				let name = "";
				if(oddNames[i] === '1')
				{
					name = home;
				}else if(oddNames[i] === '2' || oddNames[i] === "two")
				{
					name = away;		
				}else{
					if((oddData[item]['name'] === "1st Half Money Line 3-Way") || (oddData[item]['name'] === "2rd Half Money Line 3-Way")){
						name = "Tie";
					}else{
						name = "Draw";
					}
				}
				str += `<div class = "col" style = "width: auto"><div>${name}</div>
				<hr style"color:green; height:1px">`;
				for(var j = 0; j< oddData[item]['odds'].length; j++){
					if(oddNames[i] == oddData[item]['odds'][j]['name']){
						str += `<span class="btn btn-third bet-option-btn col" style="width:100%;" event_id = "${event_id}" id="bet-option-btn-${oddData[item]['odds'][j]['id']}" odd_id = "${oddData[item]['odds'][j]['id']}" price="${oddData[item]['odds'][j]['odds']}" bet="${name}" bet_name = "${oddData[item]['name']}">
						<span style="color:yellow"> ${oddData[item]['odds'][j]['odds']}</span>
					</span>	`;
					}
				}
				str += `</div>`;
			...
			for(var i = 0; i< oddData[item]['odds'].length; i++){
				let count = 0;
				for(var j = 0; j < oddNames.length; j++){
					if(oddNames[j] == oddData[item]['odds'][i]['header']){
						count += 1;
						break;
					}
				}
				if(count == 0){
					oddNames.push(oddData[item]['odds'][i]['header']);
				}
				// str +=	`<div style="color:yellow"> ${oddData[item]['odds'][i]['odds']}</div>`;
			}
			...
			for(let i = 0; i < oddNames.length; i++){
				...
				if(header){
					str += `<div class = "col" style = "width: auto"><div class = "marking-header" style = "height: 20px; text-align: center;">${header ? header : ''}</div>
						<hr style"color:green; height:1px">`;
					for(var j = 0; j< oddData[item]['odds'].length; j++){
						if(oddNames[i] == oddData[item]['odds'][j]['header']){
							str += `<span class="btn btn-secondary bet-option-btn" style="width:100%;" event_id = "${event_id}" id="bet-option-btn-${oddData[item]['odds'][j]['id']}" odd_id = "${oddData[item]['odds'][j]['id']}" price="${oddData[item]['odds'][j]['odds']}" bet="${oddData[item]['odds'][j]['name']+" - "+header+" "+oddData[item]['odds'][j]['handicap']}" bet_name = "${oddData[item]['name']}">	
										<span style="display:block">${oddData[item]['odds'][j]['name'] ? oddData[item]['odds'][j]['name'] : ''+" - "+header+" "+oddData[item]['odds'][j]['handicap']} <span style="color:yellow"> ${oddData[item]['odds'][j]['odds']}</span></span>
									</span>`;
						}
					}
				}
				else{
					str += `<div class = "col" style = "width: auto"><div></div>`;
					for(var j = 0; j< oddData[item]['odds'].length; j++){
						if(oddNames[i] == oddData[item]['odds'][j]['header']){
							str += `<span class="btn btn-secondary bet-option-btn" style="width:100%;" event_id = "${event_id}" id="bet-option-btn-${oddData[item]['odds'][j]['id']}" odd_id = "${oddData[item]['odds'][j]['id']}" price="${oddData[item]['odds'][j]['odds']}" bet="${oddData[item]['odds'][j]['name']+" - "+header+" "+oddData[item]['odds'][j]['handicap']}" bet_name = "${oddData[item]['name']}">	
										<span style="display:block">${oddData[item]['odds'][j]['name'] ? oddData[item]['odds'][j]['name'] : ''+" - "+header+" "+oddData[item]['odds'][j]['handicap']} <span style="color:yellow"> ${oddData[item]['odds'][j]['odds']}</span></span>
