...
function setOddsData(event_id,home,away,key)
{
	...
	for (const item in oddData) {
  		console.log(item)
		str += `<div class="cat-containerR" style="background-color:#585858; color: #fff ; border-top: 2px solid #367a65; padding-top: 10px"> 
					<span style="padding:15px;font-weight:700;font-size: 16px"> ${oddData[item]['name']} </span>
		...
		else if((oddData[item]['name'] === "1st Half Money Line 3-Way") || (oddData[item]['name'] === "2rd Half Money Line 3-Way"))
		{
			...
			for(var i = 0 ; i < oddData[item]['odds'].length ; i++)
			{
				...
				str += `
					<span style="display:block">${oddData[item]['odds'][i]['name']+" - "+header+" "+handicap} <span style="color:yellow"> ${oddData[item]['odds'][i]['odds']}</span></span>
				</span>	</div>`;
			}
		}
