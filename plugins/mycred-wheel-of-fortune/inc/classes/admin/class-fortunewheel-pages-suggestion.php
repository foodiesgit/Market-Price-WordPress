<?php
add_action( 'admin_footer', 'fortunewheel_pages_suggestion_javascript' );
add_action( 'wp_ajax_fortunewheel_pages_suggestion', 'fortunewheel_pages_suggestion_callback');
add_action( 'wp_ajax_nopriv_fortunewheel_pages_suggestion', 'fortunewheel_pages_suggestion_callback' );

function mycred_fortunewheel_pages_suggestion_javascript() { ?>
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                var words_type_count = 0;

                jQuery('.fortunewheel_pages_list input').before('<div class="pages-boxes"></div>');

                get_all_pages_boxes();

                jQuery('.fortunewheel_pages_list input').keyup(function (e) {
                    ...
                    var data = {
                        'action': 'fortunewheel_pages_suggestion',
                        'search': getid
                    };
                    ...
                    jQuery.post(ajaxurl, data, function (response) {
                        ...
                        jQuery('.fortunewheel_pages_list input').after('<div class="pages-suggestion">' + response + '</div>');
                        ...
                        jQuery('.pages-suggestion ul li').on('click', function () {
                            ...
                            var search = jQuery('.fortunewheel_pages_list_hidden input').val();
                            ...
                            jQuery('.fortunewheel_pages_list_hidden input').val(search_words_all);
                            jQuery('.fortunewheel_pages_list input').val('');
                ...
                jQuery('.fortunewheel_pages_list input').blur(function () {
                    setTimeout(function () {
                        jQuery('.pages-suggestion').remove();
                    }, 1000);
                });
            ...
            function remove_page_from_list() {
                jQuery('.pages-boxes .page-name span.remove').live('click', function () {
                    ...
                    var pages_list = jQuery('.fortunewheel_pages_list_hidden input').val();
                    var updated_page = pages_list.replace(remove_page, '');
                    jQuery('.fortunewheel_pages_list_hidden input').val(updated_page);
                });
            }
...
function mycred_fortunewheel_pages_suggestion_callback() {
    global $post;
